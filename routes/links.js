...
router.get('/', (req, res, next) => {
    ...
    if (str.length == '')
        ...
        arrayPages = ['Google Scholar', 'Scielo', 'Dialnet', 'Scholarpedia', 'Springlink', 'Redlatinos', 'Reefseek', 'Cern', 'Wiley', 'Microsoft', 'Base', 'Wws', 'Eric', 'Scienceresearch', 'Iseek', 'Sciencedirect', 'Elsevier', 'Wikipedia', 'Cienciasciencegov', 'Highbeam', 'Redalyc'];
        var enlaces = new Array(1000);

        const paginasPrincipal = {
            'Google Scholar': 'https://scholar.google.es/scholar?hl=es&as_sdt=0%2C5&q=',
            'Scielo': 'https://search.scielo.org/?q=',
            ...
            'Microsoft': 'https://academic.microsoft.com/#/search?iq=@',
            ...
            'Cienciasciencegov': 'https://ciencia.science.gov',
        ...
        const paginasComplemento = {
            'Google Scholar': '&btnG=',
            'Scielo': '&lang=es&count=15&from=0&output=site&sort=&format=summary&fb=&page=1&q=',
            'Dialnet': '',
            ...
            'Wiley': '',
            'Microsoft': '&filters=&from=0&sort=0',
        ...
        for (let i = 0; i < arrayPages.length; i++) {
            for (let k = 0; k < 1000; k++) {
                ...
                if (arrayPages[i] === 'Microsoft' && num[k] != undefined) {
                    paginasPrincipal[arrayPages[i]] = paginasPrincipal[arrayPages[i]] + num[k] + '%20';
                }
        ...
        let pagina = '<!doctype html><html><head><link rel="Shortcut Icon" href="images/favicon-16x16.png" type="image/x-icon" />'
        pagina += '<link rel="shortcut icon" href="images/favicon.ico" type="image/x-icon"></head><body>';
        for (let mostrar = 0; mostrar < paginaFinal.length; mostrar++) {
            pagina += `<div style="text-align: center"><a href="${paginaFinal[mostrar]}" target="_blank">${arrayPages[mostrar]}</a></div> `
        }
        pagina += '</body></html>';
        res.send(pagina);
    }
